...
**2.1 ¿Cuál es la función principal de la capa de acceso a la red?**
    ...
    La función principal de la capa de acceso a la red (también conocida como capa de enlace/física o de interfaz de red) es el intercambio de datos entre el sistema final (como una computadora o servidor) y la red física a la que está conectado. Se encarga de aislar a las capas superiores de las especificidades del hardware, gestionando el direccionamiento físico (como las direcciones MAC), el enrutamiento de los datos dentro de la misma red física o enlace directo, y el control de acceso al medio de transmisión.
...
**2.2. ¿Qué tareas realiza la capa de transporte?**
    ...
    * Distinguir las distintas aplicaciones simultáneas que se ejecutan en un host mediante el uso de Puntos de Acceso al Servicio (SAP), comúnmente conocidos como "puertos".
...
**2.4. ¿Qué es una unidad de datos del protocolo (PDU)?**
    ...
    Una PDU (Protocol Data Unit) es un bloque de información que se intercambia entre entidades pares en una arquitectura de protocolos. Cada capa toma los datos de la capa superior y les añade su propia información de control específica (cabecera o *header*). El conjunto de los datos de usuario encapsulados más la información de control de esa capa específica conforma la PDU (por ejemplo, el segmento en la capa de transporte, el datagrama en la capa de red, y la trama en la capa de acceso a la red).
...
```Plaintext
NIVEL 3    [PM China] <--------------------(Acuerdo / Ideas)------------------> [PM Francia]
               |                                                                    |
               | (Habla en Chino)                                                   | (Habla en Francés)
               v                                                                    v
NIVEL 2    [Traductor Chino-Inglés] <------(Idioma común: Inglés)-----> [Traductor Francés-Inglés]
               |                                                                    |
               | (Señales de voz en inglés)                                         | (Señales de voz en inglés)
               v                                                                    v
NIVEL 1    [Teléfono/Red Física] <=========(Señales Eléctricas)=======> [Teléfono/Red Física]
```
...
```Plaintext
           ...
           (CHINA)                           (ALEMANIA - Nodo Intermedio)                    (FRANCIA)

NIVEL 3  [PM China] <-------------------------(Acuerdo / Ideas)----------------------------> [PM Francia]
             |                                                                                   |
         (Chino)                                                                             (Francés)
             v                                                                                   v
NIVEL 2  [Trad. Chino-Japonés] <---(Japonés)---> [Trad. Jap-Alemán] <---(Alemán)---> [Trad. Alemán-Francés]
             |                                       |        |                                  |
             |                                       v        v                                  |
NIVEL 1  [Teléfono] <======(Red/Señal)=======> [Teléfono]  [Teléfono] <======(Red/Señal)=====> [Teléfono]
```
